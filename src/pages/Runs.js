...
const RUST_PDF_SERVICE = 'http://localhost:8004';
...
// ── PDF Export Button ──
function PDFExportButton({ run, isMobile }) {
  ...
  const exportPDF = async (e) => {
    ...
    const markdown = `# Clinical Agent Report\n\n## Run Summary\n- Run ID: ${run.run_id}\n- Status: ${run.status?.toUpperCase()}\n- Started: ${shortDatetime(run.started_at)}\n- Duration: ${duration(run.started_at, run.finished_at) || '—'}s\n\n## Pipeline Results\n- Papers Processed: ${run.papers_processed ?? 0}\n- Claims Extracted: ${run.claims_extracted ?? 0}\n- Conflicts Found: ${run.conflicts_found ?? 0}\n\n## Node Execution\n${PIPELINE_NODES.map(n => `- ${n.label}: ${n.time} [${n.lang}]`).join('\n')}\n\n## System Info\n- Model: Groq LLaMA 3.3 70B\n- Similarity: Rust :8003 Hybrid BM25+Cosine\n- Scheduler: APScheduler 02:00 UTC\n- Recovery: Enabled\n`;

    try {
      const res = await fetch(`${RUST_PDF_SERVICE}/render`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          run_id: run.run_id,
          title: `Run ${run.run_id?.slice(0, 8)}`,
          papers: run.papers_processed ?? 0,
          claims: run.claims_extracted ?? 0,
          conflicts: run.conflicts_found ?? 0,
          duration: duration(run.started_at, run.finished_at) ?? 0,
          markdown,
        }),
      });
      ...
      a.href = url;
      a.download = `clinical-agent-run-${run.run_id?.slice(0, 8)}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
      setState('success');
      setTimeout(() => setState('idle'), 2000);
    } catch (err) {
      // Service not running — fallback to JSON
      setState('unavailable');
      setTimeout(() => setState('idle'), 2000);
    }
  };
  ...
  const exportJSON = (e) => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `run_${run.run_id?.slice(0, 8)}.json`;
    a.click();
  };

  const configs = {
    idle:        { label: '⬇ PDF',        color: '#a78bfa', border: 'rgba(167,139,250,0.3)', bg: 'rgba(167,139,250,0.06)' },
    loading:     { label: '◌ RENDERING',  color: '#52525b', border: '#27272a',               bg: 'transparent' },
    success:     { label: '✓ DOWNLOADED', color: '#22c55e', border: 'rgba(34,197,94,0.3)',   bg: 'rgba(34,197,94,0.06)' },
    error:       { label: '✗ ERROR',      color: '#ef4444', border: 'rgba(239,68,68,0.3)',   bg: 'rgba(239,68,68,0.06)' },
    unavailable: { label: '⬇ JSON',       color: '#52525b', border: '#27272a',               bg: 'transparent' },
  };
  ...
  return (
    <div style={{ display: 'flex', gap: 4 }}>
      <button
        onClick={state === 'unavailable' ? exportJSON : exportPDF}
        ...
        style={{
          ...
          cursor: state === 'loading' ? 'not-allowed' : 'crosshair',
          transition: 'all 0.2s',
      ...
      {!isMobile && (
        <button
          ...
          style={{
            ...
            padding: '5px 12px', cursor: 'crosshair',
            transition: 'all 0.2s',
          }}
...
function RunCardMobile({ run, index, prev, isFastest }) {
  ...
  return (
    <div style={{ background: '#111113', borderLeft: `3px solid ${st.color}`, borderBottom: '1px solid #1c1c1e', animation: `rn-fadein 0.3s ${index * 0.04}s ease both`, opacity: 0 }}>
      <div onClick={() => setExpanded(!expanded)} style={{ padding: '14px', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {isFastest && <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: '#f59e0b', border: '1px solid rgba(245,158,11,0.4)', padding: '2px 5px' }}>⚡</span>}
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          {dur && <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: dur < 60 ? '#22c55e' : '#f59e0b', border: `1px solid ${dur < 60 ? 'rgba(34,197,94,0.2)' : 'rgba(245,158,11,0.2)'}`, padding: '3px 8px' }}>{dur}S</span>}
        ...
        <div style={{ borderTop: '1px solid #1c1c1e', padding: '12px 14px', background: '#0a0a0c' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 14 }}>
            {PIPELINE_NODES.map(n => (
              <div key={n.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ width: 5, height: 5, borderRadius: '50%', background: n.color, boxShadow: `0 0 4px ${n.color}66`, flexShrink: 0 }} />
          ...
          <div style={{ background: '#111113', padding: '10px 12px', borderLeft: '2px solid #22c55e', marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: '#71717a', marginBottom: 2 }}>FAITHFULNESS HEALTH</div>
          ...
          {/* Export buttons */}
          <PDFExportButton run={run} isMobile={true} />
        </div>
      )}
    </div>
  );
    ...
    >
      <div onClick={() => setExpanded(!expanded)} style={{ padding: '14px 20px', cursor: 'crosshair', display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ flex: 1, display: 'flex', gap: 14, fontFamily: 'var(--font-mono)', fontSize: 11, alignItems: 'center' }}>
          ...
          {(run.conflicts_found ?? 0) > 0 && <span style={{ color: '#ef4444', fontWeight: 600 }}>{run.conflicts_found} CONFLICTS</span>}
        ...
        <div style={{ borderTop: '1px solid #1c1c1e', padding: '14px 20px', background: '#0a0a0c' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 12 }}>
            ...
            <div>
              ...
              <div style={{ marginTop: 10, background: '#111113', padding: '10px', borderLeft: '2px solid #22c55e' }}>
          ...
          <PDFExportButton run={run} isMobile={false} />
        </div>
      )}
    </div>
  );
...
function RunHeatmap({ runs }) {
  ...
  return (
    <div style={{ background: '#111113', padding: '12px 16px', marginBottom: 2 }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: '#71717a', letterSpacing: '0.15em', marginBottom: 8 }}>30-DAY EXECUTION HEATMAP</div>
      <div style={{ display: 'flex', gap: 3, alignItems: 'flex-end' }}>
        {days.map(day => {
          const count = runMap[day] || 0;
          const isToday = day === today.toISOString().slice(0, 10);
          return <div key={day} style={{ flex: 1, height: count > 0 ? 16 : 8, background: count > 1 ? '#22c55e' : count === 1 ? '#22c55e66' : '#1c1c1e', border: isToday ? '1px solid #ef4444' : '1px solid transparent', transition: 'height 0.3s' }} />;
        })}
      </div>
...
export default function Runs() {
  ...
  return (
    <div style={{ padding: isMobile ? '16px 0' : '20px 28px', maxWidth: 1200, background: '#09090b', minHeight: '100vh' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 10 }}>
          <div>
            ...
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: isMobile ? 9 : 10, color: '#71717a', marginTop: 8, letterSpacing: '0.1em', lineHeight: 1.6 }}>
              FULL EXECUTION HISTORY · ~40S/RUN · 97× SPEEDUP · PDF EXPORT VIA RUST :8004
            </div>
          </div>
          {runs.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ fontFamily: "'Bebas Neue','Barlow Condensed',sans-serif", fontSize: isMobile ? 28 : 32, color: rate > 90 ? '#22c55e' : '#f59e0b', lineHeight: 1, textShadow: rate > 90 ? '0 0 10px rgba(34,197,94,0.4)' : 'none' }}>{rate}%</div>
              <div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: '#71717a', letterSpacing: '0.1em' }}>SUCCESS RATE</div>
      ...
      {runs.length > 0 && <div style={{ padding: isMobile ? '0 0' : '0' }}><RunHeatmap runs={runs} /></div>}

      {/* Reliability banner */}
      <div style={{ background: '#111113', borderLeft: '2px solid #22c55e', padding: '8px 14px', marginBottom: 2 }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(5, 1fr)', gap: isMobile ? 6 : 12 }}>
          {[
            ...
            <div key={label} style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: '#71717a', letterSpacing: '0.08em' }}>{label}:</span>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color, letterSpacing: '0.06em' }}>{val}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Runs list */}
      {loading ? (
        <div style={{ padding: '2rem 0', fontFamily: 'var(--font-mono)', fontSize: 11, color: '#71717a', letterSpacing: '0.1em' }}>LOADING...</div>
      ) : error ? (
        <div style={{ padding: '2rem 0', fontFamily: 'var(--font-mono)', fontSize: 11, color: '#ef4444' }}>ERR: {error}</div>
      ) : runs.length === 0 ? (
        <div style={{ padding: '3rem 0', fontFamily: 'var(--font-mono)', fontSize: 11, color: '#52525b', letterSpacing: '0.15em' }}>/ SYSTEM IDLE /</div>
      ) : isMobile ? (
